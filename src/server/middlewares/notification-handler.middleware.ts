...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'OrderCreated.create': {
    roles: ['business-owner', 'store-manager', 'store-employee', 'customer-service-representative'],
    key: 'order_id',
    tenantPath: [],
    userPath: [],
  },
...
const ownerRoles: string[] = ['store-employee'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.lead.findFirst({
