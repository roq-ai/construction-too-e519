...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<LeadInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/leads` : null),
    () => getLeads({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Leads',
      path: isTenantUser ? `/leads/view/${data?.[0]?.id}` : '/leads',
      entity: 'lead',
